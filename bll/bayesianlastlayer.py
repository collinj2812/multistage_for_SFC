...
class LogMarginalLikelihood:
    ...
    def lml(self, x, y, training=False):
        ...
        # issue with tf.linalg.logdet, use numpy instead, unfortunately @tf.function does not support numpy
        _, logdet = np.linalg.slogdet(Lambda_p_bar.numpy())
        J += self.n_y/(2*m) * tf.convert_to_tensor(logdet, dtype=tf.float32)
