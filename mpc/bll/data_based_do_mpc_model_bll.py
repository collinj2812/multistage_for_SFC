...
def data_based_model(casadi_NN, casadi_NN_std, l, no_states, no_inputs, opt_inputs):
    ...
    # uncertainty values
    uncertainty = model.set_variable('_p', 'uncertainty', shape=no_states)
    ...
    model.set_expression('test_plus', x_next + 2 * std)
    model.set_expression('test_minus', x_next - 2 * std)
    model.set_expression('test', x_next)
    model.set_expression('test_uncertainty', uncertainty)

    # define rhs
    model.set_rhs('x_k-0', x_next + uncertainty * 2 * std)
