...
class MPC_CQR:
    def __init__(self, sfc_model, data, cqr_model):
    ...
    def setup_mpc(self, n_horizon, t_step, bounds, l):
        ...
        self.db_model = data_based_do_mpc_model_cqr.data_based_model(self.cqr_model.casadi_model, self.cqr_model.casadi_model_lower, self.cqr_model.casadi_model_upper,  self.data.l, no_states=len(self.data.keys_states), no_inputs=len(self.data.keys_inputs), opt_inputs=self.data.keys_inputs)
        ...
        self.controller.settings.n_robust = 1
        ...
        # p values
        self.n_combinations = 3

        p_template = self.controller.get_p_template(self.n_combinations)

        p_template['_p', :, 'uncertainty_mean'] = [1, 0, 0]
        p_template['_p', :, 'uncertainty_upper'] = [0, 1, 0]
        p_template['_p', :, 'uncertainty_lower'] = [0, 0, 1]

        def p_fun(t_now):
            return p_template

        self.controller.set_p_fun(p_fun)
